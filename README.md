...
- `index:batch` — батч‑индексация N видео канала в таблицу канала.
  ...
  - Прогресс: `--progress on|off` (по умолчанию `on`, если настроены `TELEGRAM_BOT_TOKEN` и `ADMIN_USER_ID`), `--progress-every <N>` (шаг уведомлений, по умолчанию `INDEX_PROGRESS_EVERY=100`). Уведомления отправляются админу в Telegram.
...
- `INDEX_PROGRESS_EVERY` — шаг уведомлений прогресса индексации для `index:batch` (по умолчанию `100`).